...
- GET  /v1/runs                    — list retained runs for one session
...
def _approval_event_choices(*, smart_denied: bool, allow_permanent: bool) -> list[str]:
    """Compatibility delegate for the approval-choice authority in RunService."""
    return RunService._approval_choices(
        {
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    @_run_streams.setter
    def _run_streams(
        self, values: Dict[str, "asyncio.Queue[Optional[Dict]]"]
    ) -> None:
        # Existing adapter tests replace this private map wholesale. Keep that
        # compatibility without introducing adapter-owned shadow state.
        self._run_service.streams = values
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        # request.json() is an await point: stop may win while the body is read.
        # Revalidate immediately before the synchronous approval side effect.
        approval_session_key = self._run_service.active_approval_session_for(
            run_id, owner_filter
        )
