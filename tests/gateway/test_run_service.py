...
def test_list_for_session_matches_exact_owner_orders_newest_first_and_limits_to_100():
    registry = RunRegistry()
    ...
    runs = registry.list_for_session("conversation-a")
...
def test_get_hides_missing_run_and_conversation_mismatch_the_same_way():
    ...
    assert registry.get("run_owned", expected_session_id="conversation-a")["run_id"] == "run_owned"
...
def test_status_snapshots_cannot_mutate_authoritative_ownership():
    ...
    status = registry.get("run_owned", expected_session_id="conversation-a")
    assert status is not None
...
def test_list_for_session_enforces_hard_limit_of_100():
    registry = RunRegistry()
    for index in range(105):
        registry.set_status(
            f"run_{index:03d}",
    ...
    assert len(registry.list_for_session("conversation-a", limit=1000)) == 100
...
def test_claim_stop_target_atomically_checks_owner_and_marks_stopping():
    registry = RunRegistry()
    agent = object()
    task = object()
    registry.set_status("run_owned", "running", session_id="conversation-a")
...
def test_claim_prevents_control_replacement_without_holding_global_lock():
    ...
    assert registry.get("run_owned", "conversation-a") is not None
...
def test_claim_defers_control_cleanup_until_release():
    ...
    assert registry.claim_stop_target("run_owned", "conversation-a") is not None
