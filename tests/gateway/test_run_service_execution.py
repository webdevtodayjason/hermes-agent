...
def test_approval_choices_follow_backend_capabilities(
    smart_denied, allow_permanent, expected
):
    service = RunService(FakeHooks())

    assert service._approval_choices(
        {
...
@pytest.mark.anyio
async def test_late_callbacks_cannot_regress_or_displace_terminal_delivery(
    result, expected_status
):
...
@pytest.mark.anyio
async def test_callbacks_during_stopping_cannot_mutate_status_or_emit_events():
    ...
    text_callback = hooks.created[-1]["stream_delta_callback"]
    tool_callback = hooks.created[-1]["tool_progress_callback"]
    stopping = service.stop(run_id, "conversation-a")
    assert stopping == {"run_id": run_id, "status": "stopping"}
    ...
    agent.release.set()
    terminal = await _wait_terminal(service, run_id)
    assert terminal["status"] == "cancelled"


@pytest.mark.anyio
async def test_nonterminal_delivery_scheduled_before_terminal_is_dropped():
    service = RunService(FakeHooks(), queue_maxsize=2, clock=lambda: 42.0)
    run_id = "run_scheduled_late"
    queue = asyncio.Queue(maxsize=2)
    service.streams[run_id] = queue
    service.registry.set_status(run_id, "running", session_id="conversation-a")
    late_event = {
        "event": "message.delta",
        "run_id": run_id,
        "timestamp": 41.0,
    ...
    service.registry.set_status(run_id, "completed", last_event="run.completed")
    terminal_event = {
        "event": "run.completed",
        "run_id": run_id,
        "timestamp": 42.0,
...
@pytest.mark.anyio
async def test_late_callbacks_after_cancellation_preserve_terminal_delivery():
    agent = FakeAgent()
    agent.block = True
    hooks = FakeHooks(agent)
    service = RunService(hooks, queue_maxsize=2)
...
@pytest.mark.anyio
async def test_nonterminal_producers_do_not_schedule_after_terminal(monkeypatch):
    hooks = FakeHooks(FakeAgent({"final_response": "finished"}))
    service = RunService(hooks, queue_maxsize=2)
    ...
    def record_delivery(callback, *args):
        if callback == service._publish_nonterminal_event:
    ...
    tool_callback("tool.started", tool_name="late", preview="late")
    approval_callback({"command": "late approval", "allow_permanent": True})
    await asyncio.sleep(0)
...
@pytest.mark.parametrize(
    "agent",
    [
        FakeAgent({"failed": True, "error": "sensitive structured failure"}),
        FakeAgent(error=RuntimeError("sensitive raised failure")),
    ],
)
@pytest.mark.anyio
async def test_redaction_failure_still_terminalizes_and_cleans_up(agent):
    hooks = FakeHooks(agent)

    def fail_redaction(value):
        raise RuntimeError("redactor failed")

    hooks.redact_error = fail_redaction
    service = RunService(hooks)
...
@pytest.mark.anyio
async def test_stop_fences_late_approval_request_and_response_updates():
    class UncooperativeAgent(FakeAgent):
        def interrupt(self, message):
            self.interrupt_calls.append(message)
    ...
    run_id = _start(service)
    assert await asyncio.to_thread(agent.started.wait, 3)
    approval_notify = hooks.registered[0][1]

    assert service.stop(run_id) == {"run_id": run_id, "status": "stopping"}
    approval_notify({"command": "private", "allow_permanent": True})
    published = service.publish_approval_response(
        run_id,
        expected_session_id="conversation-a",
        choice="once",
        resolved=1,
